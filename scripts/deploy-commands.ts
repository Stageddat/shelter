...
// Configuración de variables de entorno (asumimos que tienes un archivo env o config)
const env = {
  DISCORD_TOKEN: process.env.DISCORD_TOKEN || "MTEzNDQ5MzAwNDQ1MTgwNzI0Mg.G1kuCI.r0l49QMXt0CRqXhFimxD7VTsytFLLIOmAcFiA4",
  DISCORD_APP_ID: process.env.DISCORD_APP_ID || "1134493004451807242",
  DISCORD_GUILD_ID: process.env.DISCORD_GUILD_ID || "843575156244348928",
};

// Definición de tipos para los comandos
...
/**
 * Carga todos los comandos slash desde la carpeta de comandos
 * @returns Un objeto con los comandos indexados por nombre
 */
      ...
      for (const filePath of slashCommandFiles) {
        try {
          ...
          if (!commandModule || !commandModule.data) {
            console.log(`[WARNING] El comando en ${filePath} falta la propiedad 'data'.`);
            continue;
          }
  
          const commandBuilder = commandModule.data;
          if (!(commandBuilder instanceof SlashCommandBuilder)) {
            console.log(`[WARNING] 'data' en ${filePath} no es un SlashCommandBuilder válido.`);
            continue;
          }
...
/**
 * Construye un SlashCommandBuilder a partir de la configuración y datos del comando
 */
function buildSlashCommand(config: SlashCommandConfig, command: SlashCommand): SlashCommandBuilder {
  ...
  if (command.permissions) commandBuilder.setDefaultMemberPermissions(command.permissions);
...
/**
 * Agrega opciones al comando según la configuración
 */
function addCommandOptions(commandBuilder: SlashCommandBuilder, options: SlashCommandOptionConfig[]) {
  options.forEach((option) => {
    switch (option.type) {
      case "STRING":
        commandBuilder.addStringOption((opt) => {
          setGenericOptionInfo(opt, option);
          if (option.choices) opt.addChoices(...(option.choices as { name: string; value: string }[]));
          return opt;
        });
        break;
      case "INTEGER":
        commandBuilder.addIntegerOption((opt) => {
          setGenericOptionInfo(opt, option);
          if (option.choices) opt.addChoices(...(option.choices as { name: string; value: number }[]));
...
/**
 * Establece propiedades genéricas para las opciones del comando
 */
...
/**
 * Lista los comandos remotos (globales o de guild)
 */
...
/**
 * Verifica comandos locales contra remotos y prepara opciones para el usuario
 */
async function verifyCommands(isGlobal: boolean): Promise<Choice[]> {
  ...
  if (remoteCommands === null) {
    return localCommandNames.map((name) => ({ title: `🆕 ${name}`, value: name }));
  }
...
/**
 * Despliega comandos seleccionados, actualizando solo los seleccionados y manteniendo los demás
 */
async function deploySelectedCommands(selectedCommands: string[], isGlobal: boolean) {
    ...
    try {
      ...
      console.log(`Se desplegaron ${selectedCommands.length} comandos con éxito.`);
    } catch (error) {
      console.error("Error desplegando comandos:", error);
    }
  }
/**
 * Despliega todos los comandos localmente de forma automática (solo global)
 */
async function deployAllCommands() {
    ...
    const rest = new REST({ version: "10" }).setToken(env.DISCORD_TOKEN);
    try {
      await rest.put(Routes.applicationCommands(env.DISCORD_APP_ID), { body: commandBuilders });
...
/**
 * Carga comandos remotos para eliminación
 */
...
async function main() {
  const actionResponse = await prompts({
    ...
    choices: [
      { title: "Registrar/actualizar comandos (global o por servidor)", value: "regUpdate" },
      { title: "Eliminar comandos (global o por servidor)", value: "delete" },
      { title: "Desplegar todos los comandos automáticamente (globalmente)", value: "deployAll" },
